...
const ShotForm = ({ initialData, isEditing = false }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        await api.createShot(shotData);
      } else {
        await api.createShot(shotData);
      }
      
      navigate('/admin/shots');
    } catch (error) {
      console.error('Shot form error:', error);
      setError(error.response?.data?.message || 'Error saving shot');
    } finally {
  ...
  return (
    <FormContainer>
      <FormTitle>{isEditing ? 'Edit Shot' : 'Add New Shot'}</FormTitle>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <FormGroup>
          ...
          />
          <InfoMessage>
            Enter a YouTube video URL. The thumbnail will be automatically extracted.
          </InfoMessage>
        ...
        <ButtonContainer>
          ...
          >
            {isSubmitting ? 'Saving...' : (isEditing ? 'Update Shot' : 'Add Shot')}
          </Button>
        </ButtonContainer>
      </form>
    </FormContainer>
